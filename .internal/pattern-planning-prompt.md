...
- [ ] `src/content/accessibility-docs/{pattern}/ja.mdx` - ARIA 仕様解説（日本語）

### Phase 4: ページ（動的ルーティングで自動生成）
- `meta.ts` が存在すれば `[pattern]/[framework]/index.astro` により 4フレームワーク × 2言語 のページが自動生成される
- [ ] `src/pages/patterns/{pattern}/index.astro` - リダイレクト（必要な場合）
...
1. **アクセシビリティドキュメント（MDX）**
   - `src/content/accessibility-docs/{pattern}/en.mdx` - 英語版
   - `src/content/accessibility-docs/{pattern}/ja.mdx` - 日本語版
   - llm.md の内容を元に詳細な解説を作成
   - frontmatter は `pattern` と `locale` のみ
...
### Phase 5: ページ（動的ルーティングで自動生成）

`meta.ts` が存在すれば `[pattern]/[framework]/index.astro` により 4フレームワーク × 2言語 のページが自動生成される。個別のページファイル作成は不要。
...
1. **成果物照合チェック**
   ...
   ls -la src/content/accessibility-docs/{pattern}/ja.mdx
   ...
   **成果物チェックリスト**:
   ...
   - [ ] src/content/accessibility-docs/{pattern}/ja.mdx
   - [ ] TestingDocs.astro
   ```
