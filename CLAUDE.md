...
1. `curl -s -o /dev/null -w '%{http_code}' http://localhost:4336/apg-patterns-examples/` でアクセスを試みる（通常このポートで起動している。base 配下を叩くこと。ルート `/` は 404）
2. 応答がなければ `lsof -i -P | grep LISTEN | grep node` で起動中のポートを探す
3. どちらも見つからなければ `npm run dev` をバックグラウンドで起動する

ポートは自動割当（cwd ハッシュベース）のため固定値ではない。**ローカルの base path も本番と同じ `/apg-patterns-examples/`**（`DEPLOY_TARGET` の既定が `github-pages` のため）。base を `/` で動かしたい場合は `DEPLOY_TARGET=cloudflare-pages npm run dev`。deploy target → base / port の解決は `scripts/deploy-target.mjs` に一元化されており、`astro.config.mjs` / `playwright.config.ts` / `scripts/axe-check.mjs` が共有する。
