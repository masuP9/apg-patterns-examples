...
| `aria-label` | treegrid | 文字列 | はい（aria-labelまたはaria-labelledbyのいずれか） | treegridのアクセシブルな名前 |
| `aria-labelledby` | treegrid | ID参照 | はい（aria-labelまたはaria-labelledbyのいずれか） | aria-labelの代替 |
...
APGはtreegridのマウス挙動を規定していない。本実装はcell-only focus modelとキーボードのセマンティクスを変えずに、最小限のマウス操作を加えている:

| 操作 | 挙動 |
| --- | --- |
| 任意のセルをクリック | そのセルへフォーカス移動（roving tabindex を更新）。行選択 (`aria-selected`) は**変更しない** |
| 展開可能な非disabled行のrowheaderをクリック | 上記に加えて展開/折りたたみをトグル |
| `aria-disabled="true"` の行のセルをクリック | フォーカスは移動するが、展開トグルは**起きない**（矢印キーの挙動と整合） |
| セル内のインタラクティブな子要素（`<button>` / `<a>` / `<input>` / `<select>` / `<textarea>`）をクリック | treegrid側はハンドリングをスキップし、子要素のデフォルト挙動に任せる（展開トグルもフォーカス移動も行わない） |
| ダブルクリック | 追加挙動なし（セルのアクティベートは引き続き `Enter` キー専用） |

行選択（`aria-selected`）は引き続き `Space` キー専用とし、APGのcell-only focus modelを保持する。
