...
| ロール | 要素 | 説明 |
| --- | --- | --- |
| `tree` | コンテナ（<code>&lt;ul&gt;</code>） | ツリーウィジェットのコンテナ |
| `treeitem` | 各ノード（<code>&lt;li&gt;</code>） | 個々のツリーノード（親ノードとリーフノードの両方） |
| `group` | 子コンテナ（<code>&lt;ul&gt;</code>） | 展開された親の子ノードのコンテナ |

### プロパティ

| 属性 | 要素 | 値 | 必須 | 備考 |
| --- | --- | --- | --- | --- |
| `role="tree"` | コンテナ | - | はい | コンテナをツリーウィジェットとして識別 |
