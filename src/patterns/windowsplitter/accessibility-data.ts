...
export const windowSplitterAccessibilityData: PatternAccessibilityData = {
  ...
  testing: {
    ...
    e2eTestFile: 'e2e/windowsplitter.spec.ts',
    commands: [
      {
        comment: {
        ...
        command: 'npx vitest run src/patterns/windowsplitter/',
      },
      {
        comment: {
      ...
      },
      {
        ...
        command: 'npm run test:e2e -- windowsplitter.spec.ts',
      },
      {
        comment: {
        ...
        command: 'npm run test:e2e:ui -- windowsplitter.spec.ts',
      },
    ],
    tools: [
      {
...
test('has role="separator" with required attributes', async ({ page }) => {
  await page.goto('/patterns/windowsplitter/react/');
