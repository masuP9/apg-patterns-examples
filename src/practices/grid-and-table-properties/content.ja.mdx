...
<CodeFigure
  ...
  lang="html"
  caption="DOM には 10〜20 行目のみレンダリングされています。aria-rowcount と aria-rowindex がデータセット全体のサイズと位置をスクリーンリーダーに伝えます。"
/>
...
<CodeFigure
  ...
  lang="html"
  caption="scope=&quot;col&quot; の th は暗黙的に columnheader ロール、scope=&quot;row&quot; の th は rowheader ロール、td は cell ロールを持ちます。"
/>
...
<CodeFigure
  code={`<div role="gridcell" aria-readonly="true">固定値</div>`}
  lang="html"
  caption="読み取り専用セル — 値を編集できません。"
/>

<CodeFigure
  code={`<div role="gridcell" aria-disabled="true">利用不可</div>`}
  lang="html"
  caption="無効なセル — インタラクションが利用できません。"
/>
...
<CodeFigure
  ...
  lang="html"
  caption="悪い例: 位置情報がないため、スクリーンリーダーがデータセット内の行位置を判断できません。"
/>

<CodeFigure
  code={`<div role="row" aria-rowindex="50">...</div>`}
  lang="html"
  caption="良い例: 明確な位置 — スクリーンリーダーが「50行目」と読み上げられます。"
/>
...
<CodeFigure
  code={`<th>名前</th>`}
  lang="html"
  caption="悪い例: scope がないため、スクリーンリーダーがヘッダーと列・行の関連付けを正しく行えない可能性があります。"
/>

<CodeFigure
  code={`<th scope="col">名前</th>
<th scope="row">太郎</th>`}
  lang="html"
  caption="良い例: 明示的な scope でヘッダーの方向（列または行）を明確にします。"
/>
