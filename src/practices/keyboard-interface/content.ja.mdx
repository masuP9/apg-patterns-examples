...
| `nomemory` | Tabは常に先頭（または `focusgroupstart`）から入り、フォーカス履歴を無視する       |
...
2026年初頭時点で、`focusgroup` はChromiumベースのブラウザ（Chrome/Edge 133+）でフラグまたはオリジントライアルとして利用可能です。Firefox や Safari ではまだサポートされていません。
