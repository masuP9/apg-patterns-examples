...
Keyboard accessibility is essential for users who cannot use a mouse, including people with motor disabilities, blind users, and power users who prefer keyboard navigation. Unlike native HTML form elements, browsers do not provide built-in keyboard support for custom widgets—authors must implement it themselves.
...
<CodeFigure
  ...
  lang="html"
  caption="Initial state: only the first button has tabindex=&quot;0&quot; and is included in the tab order."
/>
...
<CodeFigure
  ...
  lang="html"
  caption="After pressing the → key: the second button now has tabindex=&quot;0&quot; and receives focus."
/>
...
<CodeFigure
  code={`<ul role="listbox" tabindex="0" aria-activedescendant="option-2" aria-label="Select a fruit">
  ...
  lang="html"
  caption="Listbox using aria-activedescendant — DOM focus stays on the ul, while option-2 (Banana) is the currently active item."
/>
...
1. **Collapse all focusable descendants into a single Tab stop** — even if multiple `<button>` elements are present, only one is reachable via Tab
2. **Enable arrow-key navigation** between focusable children within the group
3. **Automatically manage roving tabindex** — the browser handles `tabindex` values internally
4. **Remember the last-focused item** — when a user Tabs away and back, focus returns to the previously focused item

<CodeFigure
  code={`<!-- Before: Manual roving tabindex (JavaScript required) -->
...
<div role="toolbar" aria-label="Formatting" focusgroup="horizontal">
  <button>Bold</button>
  <button>Italic</button>
  <button>Underline</button>
</div>`}
  lang="html"
  caption="The focusgroup attribute replaces manual roving tabindex management. The browser automatically collapses the buttons into a single Tab stop and enables arrow-key navigation."
/>
...
<CodeFigure
  code={`<!-- Horizontal toolbar that wraps -->
...
<!-- Vertical menu -->
<ul role="menu" focusgroup="vertical">
  <li role="menuitem" tabindex="0">File</li>
  <li role="menuitem" tabindex="-1">Edit</li>
  <li role="menuitem" tabindex="-1">View</li>
</ul>

<!-- 2D grid navigation (e.g., emoji picker, calendar) -->
...
<CodeFigure
  code={`<!-- Independent nested focusgroups -->
...
<div focusgroup="horizontal">
  ...
  <div focusgroup="extend horizontal">
    <!-- Same scope as parent: A1 → B1 → B2 → A2 -->
    <button>B1</button>
    <button>B2</button>
  </div>
  <button>A2</button>
...
The `focusgroup` attribute works across Shadow DOM boundaries by default. A focusgroup declared on a shadow host includes focusable elements inside that host's shadow tree. You can opt out with `focusgroup="none"` inside the shadow tree if needed.
...
<FocusgroupComparisonTable locale="en" />
...
<script>
  ...
  lang="html"
  caption="Progressive enhancement: use focusgroup where supported, fall back to JavaScript-based roving tabindex elsewhere."
/>
