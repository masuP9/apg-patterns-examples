...
<CodeFigure
  code={`<img src="hero.jpg" alt="画像" />`}
  lang="html"
  caption="悪い例: 冗長 — ロールは自動的に伝えられるため「画像 画像」と読み上げられます。"
/>

<CodeFigure
  code={`<img src="hero.jpg" alt="夕焼けに染まる東京タワー" />`}
  lang="html"
  caption="良い例: 画像の内容を具体的に説明する名前です。"
/>
...
<CodeFigure
  ...
  lang="html"
  caption="悪い例: 非インタラクティブ要素に aria-label — 段落はアクセシブルな名前の計算で aria-label の対象外のため、「重要」は無視され「これは段落です」がそのまま読み上げられます。"
/>

<CodeFigure
  code={`<p><strong>重要:</strong> これは段落です</p>`}
  lang="html"
  caption="良い例: 強調したい情報はテキストコンテンツとして含め、段落のテキストがそのままアクセシブルなコンテンツになります。"
/>
...
4. `title` 属性（本来の目的が名前付けではないため、アクセシブルな名前として質が低くなりがち。できる限り避ける）
