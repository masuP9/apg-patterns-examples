...
- Roving tabindex (only the selected element has tabIndex=0)
...
The Container API runs in a Node.js environment where `HTMLElement` is not defined,
so it cannot test the client-side behavior of the Web Component.
...
// ✅ Stable: use element.press()
await secondTab.click();
await secondTab.press('ArrowLeft'); // sends the key directly to the element
```

**Countermeasure**:
- Use `element.press()` instead of `page.keyboard.press()`
- `element.press()` sends the key event directly to the element, so it is unaffected by focus races
...
**Countermeasure**:
- `focus()` the selected tab (`tabindex="0"`) first, then navigate to the target tab with the keyboard
- Only call `focus()` directly on elements that have `tabindex="0"`
...
// Is the ID set correctly? (a hydration indicator)
...
|---------|------|--------|
| `element.focus()` | Set focus on an element | ✅ only for `tabindex="0"` elements |
| `element.press(key)` | Send a key directly to an element | ✅ stable |
| `element.click()` | Click an element | ⚠️ watch out for keyboard.press() after focus moves |
...
test('ArrowRight moves focus to next tab', async ({ page }) => {
  ...
  // 2. Send the key with element.press()
...
- [Playwright Locators](https://playwright.dev/docs/locators) — the difference between `element.press()` and `page.keyboard.press()`
